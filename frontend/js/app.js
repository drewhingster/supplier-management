...
function openSupplierModal(supplier = null) {
    ...
    CONFIG.DOCUMENT_TYPES.forEach(docType => {
        ...
        const viewBtn = document.querySelector(`.doc-view[data-doc="${docType.id}"]`);
            ...
            CONFIG.DOCUMENT_TYPES.forEach(docType => {
                ...
                if (doc) {
                    ...
                    if (viewBtn) {
                        ...
                        viewBtn.onclick = () => viewDocument(supplier.id, docType.id);
                    }
                }
            });
        }
...
async function openDetailModal(supplier) {
    ...
    CONFIG.DOCUMENT_TYPES.forEach(docType => {
        ...
        if (isUploaded) {
            card.onclick = () => viewDocument(supplier.id, docType.id);
        }
        ...
        card.innerHTML = `
            <svg viewBox="0 0 24 24" class="doc-icon">
                ...
                ${isUploaded ? '<polyline points="9,15 12,18 17,13" fill="none" stroke="currentColor" stroke-width="2"/>' : ''}
            </svg>
            <div class="doc-details">
                <span class="doc-label">${docType.name}</span>
                <span class="doc-status ${isUploaded ? 'uploaded' : 'missing'}">
                    ${isUploaded ? 'Click to view' : 'Not uploaded'}
                </span>
            </div>
        `;
...
async function viewDocument(supplierId, docType) {
    try {
        const response = await api.getDocumentUrl(supplierId, docType);
        if (response.url) {
