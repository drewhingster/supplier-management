...
class SupplierAPI {
     ...
     * @param {number} supplierId - Supplier ID
     * @param {string} documentType - Type of document
     * @returns {string} Direct URL to the document
     */
    getDocumentDirectUrl(supplierId, documentType) {
        const token = this.getToken();
        // Include auth token as query parameter for direct access
        // The worker will need to accept this for document viewing
        return `${this.baseUrl}/suppliers/${supplierId}/documents/${documentType}?token=${encodeURIComponent(token)}`;
    }

    /**
     * Get document download URL (legacy - now returns direct URL)
     * @param {number} supplierId - Supplier ID
     * @param {string} documentType - Type of document
     * @returns {Promise<Object>}
     */
